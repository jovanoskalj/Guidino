...
def perspective(frame):
    for i in range(4):
        pt1 = tuple(map(int, source[i]))
        pt2 = tuple(map(int, source[(i+1)%4]))
...
# Main loop
# -----------------------------
command_names = {0: "FORWARD", 1:"RIGHT1",2:"RIGHT2",3:"RIGHT3",4:"LEFT1",5:"LEFT2",6:"LEFT3",7:"UTURN"}
...
try:
    while True:
        ...
        # STOP → U-turn
        if horizontal_line_detected and not stop_triggered:
            send_command(0)  # STOP
            stop_triggered = True
            stop_time = time.time()
            print("Horizontal line detected - preparing U-turn")
            continue
        
        if stop_triggered and time.time() - stop_time > 1.5:
            # Send U-turn sequence to Arduino
            for cmd in [6,0,6,0]:  # Example: LEFT3 → FORWARD → LEFT3 → FORWARD
                send_command(cmd)
                time.sleep(0.5)
            stop_triggered = False
            last_command = -1  # Reset last command after U-turn
            continue
